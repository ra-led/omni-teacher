...
2. Populate any required secrets in a `.env` file (see `docker-compose.yml` for environment variables). At minimum set
   `OPENAI_API_KEY` and, if you use a custom endpoint, `OPENAI_API_BASE` (defaults to `https://api.openai.com/v1`). For
   OpenRouter-style speech recognition, set `OPENAI_BASE_URL=https://openrouter.ai/api/v1` and
   `OPENAI_TRANSCRIBE_MODEL=xiaomi/mimo-v2-omni` or another OpenRouter audio-capable model. The
   ...
   browser allow-list, adjust `CORS_ORIGINS` (comma separated) to match the hostnames you use during development. For
   audio playback outside the container network, set `MINIO_PUBLIC_ENDPOINT` (e.g. `http://localhost:9000`) so the
   backend returns browser-routable URLs instead of the internal `minio` hostname.
3. Start the stack:
   ```bash
